...
%   display is suppressed (no flickering, silent operation).
%   Set to TRUE to open figure windows on screen (for debugging/inspection).
%   Easy toggle for future interactive use.
...
%  Injecting in the +I (eastward / left-in-image) direction forces the CO2
%  plume to travel the full formation length before reaching the updip
%  boundary, maximising residual and dissolution trapping along the way.
%
%  Method: patch model.operators.T (the half-transmissibility array that
%  TwoPhaseWaterGasModel assembled internally) by zeroing out every
%  inter-cell face of each directional injector cell that does NOT point
...
%  fail with multiple injectors.  This block manually derives the same
%  physical categories directly from the simulation states:
%
%    Total injected   : cumulative CO2 mass from wellSol (all injectors)
%    Free plume       : mobile CO2 still in formation  (s_co2 > src)
%    Residual         : immobile CO2 trapped by capillarity (src * poro * vol)
%    Exited           : mass balance residual (injected - free - residual)
% =========================================================================
...
%  (SBoundary_test_well .. _4). Those sampled only 4 hand-picked (I,J)
%  locations along the western extreme of the reservoir footprint, which
%  left plenty of room for the CO2 plume to bypass between them.
%
%  Instead, we now scan EVERY J-row of the grid and find the westernmost
%  active cell (i.e. the true edge of the Johansen formation footprint at
%  that row — this naturally traces the actual, possibly irregular,
%  boundary shape instead of relying on 4 sparse hand-picked points).
%
%  TOP_LAYER = 6 is the shallowest layer of the old 6:10 perforation range
%  the surveillance wells used to sample — i.e. "the layer in which all
%  those wells were perforated", now used directly instead of averaging
%  over 5 layers.
%
%  For each boundary cell, we check the CO2 gas saturation at the END of
%  the simulation against the 15% threshold and write a flat CSV:
%     cell_x, cell_y, breach   (breach = 1 if S_CO2 > 0.15, else 0)
%
%  The Python BO notebook now reads this CSV directly for Condition 2
%  (CO2 plume boundary breach) instead of parsing 4 separate well CSVs.
% =========================================================================
